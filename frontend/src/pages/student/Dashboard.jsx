...
const StudentDashboard = () => {
  ...
  }, [profile?.id]); // Pastikan dipanggil saat ID tersedia
  ...
  useEffect(() => {
    if (profile?.id) {
        ...
        // Subscription Realtime untuk Leaderboard (jika ada teman sekelas yang XP-nya update)
  ...
  const fetchLeaderboard = async () => {
    ...
    try {
      // Mengambil SEMUA siswa di kelas yang sama agar transparan (tanpa limit 5)
      let query = supabase
        .from('profiles')
        .select('id, full_name, xp')
        .eq('role', 'siswa');

      // Jika ada kode kelas, filter peringkat hanya untuk teman sekelas
      if (profile.class_code) {
      ...
      <main className="flex-1 p-6 md:p-10 lg:p-14 max-w-7xl mx-auto w-full overflow-y-auto h-screen custom-scrollbar relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-10">
          <div className="lg:col-span-8 space-y-12">
            ...
            {/* XP PROGRESS BAR */}
            <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 relative overflow-hidden group">
               ...
               <div className="relative h-5 bg-slate-100 rounded-full overflow-hidden border-4 border-slate-50 shadow-inner">
                  <motion.div
                    key={profile?.xp} // Force re-animation when XP changes
                    initial={{ width: 0 }}
          ...
          <div className="lg:col-span-4 space-y-10">
            ...
            <section className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8">
               ...
               <Link to="/student/tasks" className="block w-full text-center mt-8 py-4 bg-slate-900 text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-indigo-600 transition-all">
                  Lihat Semua
               </Link>
            </section>
          </div>
