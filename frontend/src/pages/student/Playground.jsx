...
const Playground = () => {
  ...
  const initChat = async () => {
    ...
    try {
      ...
      if (topics && topics.length > 0) {
        const topicMap = { matematika: 'Matematika 📐', ipa: 'IPA 🔬', ips: 'IPS 🌏', english: 'English 🇬🇧' };
        welcomeText = `Halo ${firstName}! Kak Bintang ingat kamu sempat kesulitan di ${topicMap[topics[0]] || topics[0]}. Mau kita belajar bareng? 😊✨`;
      }

      setMessages([{ role: 'ai', text: welcomeText, id: Date.now(), timestamp: new Date() }]);
      setInitialized(true);
    } catch (error) {
      setMessages([{ role: 'ai', text: "Halo! Kak Bintang siap membantu. Ada pertanyaan?", id: Date.now(), timestamp: new Date() }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'ai', text: "Waduh, koneksi Kak Bintang terputus. Coba lagi ya!", id: Date.now() + 2, timestamp: new Date() }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    // Overflow hidden pada container utama untuk mencegah seluruh halaman ter-scroll
    <div className="flex h-screen w-full bg-slate-50 overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-white relative">
        ...
        <header className="h-20 px-6 border-b border-slate-100 flex items-center justify-between shrink-0 bg-white/80 backdrop-blur-md z-10">
          ...
          <button
            onClick={() => confirm('Reset percakapan?') && setMessages([])}
        ...
        {/* Chat Area: Ini yang bisa di-scroll */}
