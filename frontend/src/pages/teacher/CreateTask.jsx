...
const TeacherCreateTask = () => {
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      ...
      if (response?.success && response.quiz) {
        ...
        setTaskData(prev => ({
          ...prev,
          title: prev.title || quiz.title,
          description: prev.description || quiz.instructions,
          duration: prev.duration || quiz.duration?.toString() || '30'
        }));

        const formatted = quiz.questions.map((q) => ({
          id: crypto.randomUUID(),
          type: q.type === 'multiple_choice' ? 'pilihan_ganda' : 'esai',
          text: q.text,
          points: q.points || (q.type === 'multiple_choice' ? 10 : 20),
          correctAnswer: q.type === 'multiple_choice' ? q.correctText : q.correct,
          feedbackCorrect: q.feedback_correct || '',
          feedbackWrong: q.feedback_wrong || '',
          difficulty: aiConfig.difficulty,
          options: q.type === 'multiple_choice' ? (q.options || []).map((opt, i) => ({
            id: crypto.randomUUID(),
            text: opt,
  ...
  const handleSubmit = async () => {
    // FIX: Tambahkan pengecekan profile untuk menghindari error "reading 'id' of null"
  ...
  // FIX: Tampilkan loading atau pesan jika profile belum siap
  if (!profile) {
      return (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="relative bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden">
              <div className="relative p-6 space-y-5">
                <div className="text-center">
                  <div className={`w-16 h-16 bg-${currentSubject?.color}-100 text-${currentSubject?.color}-600 rounded-2xl flex items-center justify-center text-3xl mx-auto mb-3`}>
                    {currentSubject?.icon}
                  </div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {subjects.map(s => (
                        <button key={s.id} onClick={() => setAiConfig({...aiConfig, subject: s.id})} className={`p-2 rounded-xl text-sm font-medium transition-all border ${aiConfig.subject === s.id ? `${s.bg} ${s.text} border-${s.color}-300` : 'bg-white border-slate-200'}`}>
                          {s.icon} {s.name}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <select value={aiConfig.gradeLevel} onChange={e => setAiConfig({...aiConfig, gradeLevel: parseInt(e.target.value)})} className="w-full p-3 bg-slate-50 rounded-xl">
                      {[1,2,3,4,5,6].map(g => <option key={g} value={g}>Kelas {g} SD</option>)}
                    </select>
                  </div>

                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500">📖 Topik <span className="text-slate-400">(opsional)</span></label>
                    <input type="text" value={aiConfig.topic} onChange={e => setAiConfig({...aiConfig, topic: e.target.value})} placeholder="Kosongkan untuk AI tentukan otomatis" className="w-full p-3 bg-slate-50 rounded-xl" />
                  </div>

                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500">⚡ Kesulitan</label>
                    <select value={aiConfig.difficulty} onChange={e => setAiConfig({...aiConfig, difficulty: e.target.value})} className="w-full p-3 bg-slate-50 rounded-xl">
                      <option value="easy">Mudah</option>
                      <option value="medium">Sedang</option>
                      <option value="hard">Sulit</option>
                    </select>
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="number" value={aiConfig.count} onChange={e => setAiConfig({...aiConfig, count: Math.min(15, Math.max(1, parseInt(e.target.value) || 5))})} min={1} max={15} className="w-full p-3 bg-slate-50 rounded-xl" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      {['multiple_choice', 'essay'].map(t => (
                        <button key={t} onClick={() => setAiConfig({...aiConfig, types: aiConfig.types.includes(t) ? aiConfig.types.filter(x => x !== t) : [...aiConfig.types, t]})} className={`flex-1 py-2 rounded-xl text-sm font-bold ${aiConfig.types.includes(t) ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500'}`}>
                ...
                <div className="flex gap-3 pt-3">
                  <button onClick={() => setShowAIModal(false)} className="flex-1 py-3 bg-slate-100 rounded-xl font-bold">Batal</button>
                  <button onClick={handleGenerateAI} disabled={aiLoading} className={`flex-1 py-3 bg-${currentSubject?.color}-600 text-white rounded-xl font-bold hover:bg-${currentSubject?.color}-700 disabled:opacity-50 flex items-center justify-center gap-2`}>
